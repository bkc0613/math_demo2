...
# -*- coding:utf-8 -*-
"""code_info
@Time    : 2020 2020/7/21 13:53
@Author  : 鲍凯辰
@File    : Q1_calculate_degree.py
"""
import pandas as pd

arr = [list() for k in range(19)]
for i in range(0, 19):
    root_data = pd.read_table('question1_edge/' + str(i) + '.txt', sep=' ', header=None)
    root = root_data.iloc[:, 0]
    for others in range(0, 19):
        degree = 0
        target_data = pd.read_table('question1_edge/' + str(others) + '.txt', sep=' ', header=None)
